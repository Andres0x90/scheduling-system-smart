...
class ChangeWritingTaskDescriptionUseCaseTest {
    @Mock
    DomainEventRepository repository;
    @Test
    public void ChangeWritingTaskDescription()
    {
        ...
        var command = new ChangeWritingTaskDescription(teacher.identity(),
                WritingTaskId.of("wtxxxx"), new Description("Other description"));
        var useCase = new ChangeWritingTaskDescriptionUseCase();
        useCase.addRepository(repository);

        var writingTaskDescriptionChanged = (WritingTaskDescriptionChanged) UseCaseHandler.getInstance().
                syncExecutor(useCase, new RequestCommand<>(command))
                .orElseThrow().getDomainEvents().get(0);

        assertEquals("Other description",writingTaskDescriptionChanged.getDescription().value());
